jQuery(document).ready(function($){
	...
	function cat_filter() {	
		...
		if($('.category-filter li').hasClass('selected')){
	 		var $this = $(this);
	 		if($('.selected input').is(':checked')) { 
	 			//alert("it's checked");
	 			var $parent = $('.selected input').parents('.main-parent');
	 			var $parent1 = $('.selected input').parents('.p-3218');
				$parent1.removeClass('hidden');
					
				$('.selected').addClass('open')
				$('.selected').parents('li').addClass('open');

				if($('a#test').hasClass('opened')){
					$('a#test').removeClass('opened');
					$('a#test').parent().parent().addClass('opener-parent');
				}
				$('.selected input').parents('.main-parent').removeClass('opener-parent');
				$('.opener-parent ul').addClass('hidden');
				$('.open.opener-parent').first().addClass('fuck');
				$('.fuck ul').first().removeClass('hidden');
				$('.selected.open').parents('.list-unstyled').first().removeClass('hidden');
	 		}
		};
	}
	...
	$(document).on('change', '.category-filter input', function(){
		...
		if( $this.prop('checked') ){
			var $parent = $this.parents('.main-parent');
			if($('.main-parent').hasClass('hidden')){
